...
import { BtnCheck, BtnRed, BtnVideo, InfArea, Lapis, LapisArea, LapisText, TextBtn } from './styles';
...
export default function Petalas() {
  ...
  const [checkPetal, setCheckPetal] = useState(true)
  const [loadingInf, setLoadingInf] = useState(true);
  const [showPetalas, setShowPetalas] = useState(null)

  const response = firestore().collection('users').doc(user.uid);

  async function update(){
    updateProgress('petal')
    setShowPetalas(null)
  }
  ...
  const { petalas } = useContext(ExerContext)
...
return (
   <ScrollView style={{backgroundColor:"#000"}}>
     ...
     <View style={{ alignItems:'center',
        ...
        <TextBtn style={{marginBottom:20}}>Pétala</TextBtn>

      <BtnRed
      onPress={()=>setShowPetalas( showPetalas === null ? 'Show' : null )}
      showBola={showPetalas}
      >
        <TextBtn>Pétala</TextBtn>
        {
          checkPetal ? (
 ...
 <LapisAnime
     ...
     <View style={{
      ...
      >
        ...
        size={30}
        color="#fff"
        name={ checkPetal ? "check-circle" : "circle" }
        />
      </BtnCheck>
     </View>
